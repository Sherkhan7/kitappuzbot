    ...
    if data[0] == 'delete' and data[-1] == 'book':
        ...
        if data[1] == 'y':
            ...
            if insert_data(get_book_data_dict(user_data), 'books'):
                ...
                reply_keyb_markup = ReplyKeyboard(back_keyboard, user[LANG]).get_markup()
                callback_query.message.reply_text('📚 ' + edit_actions_btn_text, reply_markup=reply_keyb_markup)
...
def back_to_editing_callback(update: Update, context: CallbackContext):
    ...
    reply_keyb_markup = ReplyKeyboard(back_keyboard, user[LANG]).get_markup()
    update.message.reply_text('📚 ' + edit_actions_btn_text, reply_markup=reply_keyb_markup)
